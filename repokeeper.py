...
reponame="local-rk"
#conffileloc="/home/tibor/devel/repokeeper/repokeeper.conf"
#print (" DEBUG mode!!!!")
...
def parse_localrepo():
	...
	files=[]
	files.extend(glob.glob(repodir+"/*pkg.tar.?z"))
	files.extend(glob.glob(repodir+"/*pkg.tar.?z"))
...
for position,package in enumerate(pkgs_tobuild):
	...
	print (" ")
	#lf.write(" ")
	for lfile in glob.glob(compiledir+"/*pkg.tar.?z"):
...
#updating repository
time.sleep(1)
print (BOLD+"* Updating local repo db file..."+UNCOLOR)
if os.path.isfile(repodir+reponame+".db.tar.gz"):
	os.remove(repodir+reponame+".db.tar.gz")
#creating new one
try:
	subprocess.call("repo-add "+repodir+reponame+".db.tar.gz "+repodir+"*pkg.tar.?z",shell=True)
	print ("   ")
